...
for xi_iter = 1 : length(xi_grd)
    ...
    subplot(4,2,2*xi_iter)
    hold on
    plot(c/10^5,BS_Lola_in,'linewidth',LineWidth)
    plot(c/10^5,SS_Lola_in,'--','linewidth',LineWidth)
    scatter(c(thresholdTypeSB)/10^5,SS_Lola_in(thresholdTypeSB),'MarkerFaceColor','red','MarkerEdgeColor','red')
    text(c(thresholdTypeSB)/10^5,SS_Lola_in(thresholdTypeSB),'$p_L^*$','VerticalAlignment','top','HorizontalAlignment','right','interpreter','latex');
    scatter(c(thresholdType)/10^5,BS_Lola_in(thresholdType),'MarkerFaceColor','red','MarkerEdgeColor','red')
    text(c(thresholdType)/10^5,BS_Lola_in(thresholdType),'$p_L^*$','VerticalAlignment','top','HorizontalAlignment','right','interpreter','latex');
    set(gca,'FontSize',FontSize)
    xlabel('Floor Price $p_L$','interpreter','latex')

    pLStoreSS(xi_iter)=c(thresholdTypeSB)
    pLStoreBS(xi_iter)=c(thresholdType)
    
    if xi_iter==1
        legend('Lola Buyer Surplus','Lola Social Surplus','interpreter','latex')
    end


    subplot(4,2,2*xi_iter-1)
    hold on
    plot(c/10^5,w,'-','LineWidth',LineWidth)
    plot(c/10^5,(vHat-c),'--','LineWidth',LineWidth)
    ylim([.95*min(min([w (vHat-c)])) 1.03*max(max([w (vHat-c)]))])
    if xi_iter==1
        legend('$w(c)$','$v(c)-c$','Interpreter','latex','Location','best')
    end
    xlabel('c'); ylabel('');
    %title('w')
    set(gca,'FontSize',FontSize)

    annotation('textbox', [0.47 0.86-(xi_iter-1)*0.22 0.1 0.1],'String',{['\xi = ' num2str(round(xi_grd(xi_iter),2))]},'EdgeColor','none','FontSize',FontSize);
